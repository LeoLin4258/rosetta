...
#### 2026-08-03 / beta.24 onboarding window lifecycle / Codex / completed

- 状态：completed
- HEAD 与工作区：基于已推送并通过 Main app CI 的 `70a1939cd6fe7120b2e90a2ab67de700788a1571`；用户已确认该提交重建的 Windows Preview 可正常启动 PDF 组件并处理同一测试文档
- 用户授权：检查并修复 onboarding；不得操作 UI，用户负责需要的手工界面验证；发布仍是三平台同源 smoke 全部通过后的最后一步
- 根因：Windows/Linux 仅关闭 `main` 时退出，用户关闭 onboarding 会留下隐藏窗口和后台进程；macOS Reopen 按窗口存在性固定优先 `main`，可能绕过未完成 onboarding
