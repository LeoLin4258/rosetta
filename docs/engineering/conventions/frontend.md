...
- 双击译文文件打开独立预览窗口，预览窗口只负责“原文 + 当前目标语言译文”的左右对照阅读、状态查看、局部重翻和导出。
- 独立译文预览窗口打开正在翻译中的译文文件时，必须持续刷新当前译文文件并显示顶部翻译状态栏，不能只展示打开瞬间的快照。
- 正在运行的翻译必须有用户可见的停止入口。停止后 UI 应退出当前翻译循环，并把当前运行批次恢复为可重试状态，不能要求用户关闭应用来摆脱卡住的模型请求。
- 控件保持克制，优先清晰和可扫描。
...
- 桌面侧边栏宽度为 `14.375rem`，接近 shadcn 默认 `16rem` 的 90%，并保持整数 CSS 像素以避免 macOS 透明窗口拖动时出现边界取整闪烁。不允许通过中间 rail 调整宽度，`SidebarRail` 当前不渲染。
