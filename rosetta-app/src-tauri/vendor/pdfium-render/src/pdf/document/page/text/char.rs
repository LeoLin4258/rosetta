...
impl<'a> PdfPageTextChar<'a> {
    ...
    /// Returns the opaque, process-local identity of the text object containing this character.
    ///
    /// The identity is stable only while the containing Pdfium document and page remain open. It
    /// must not be persisted or interpreted as a PDF object number.
    pub fn text_object_identity(&self) -> Result<usize, PdfiumError> {
        let object_handle = unsafe {
            self.bindings()
                .FPDFText_GetTextObject(self.text_page_handle(), self.index)
        };
