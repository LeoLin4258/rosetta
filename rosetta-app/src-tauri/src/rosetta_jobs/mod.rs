...
const LIGHTNING_PDF_LARGE_RUN_PAGE_THRESHOLD: usize = 30;
...
pub fn get_rosetta_pdf_v3_run_status(
    ...
    let limit = limit.unwrap_or(formats::pdf::v3_control::DEFAULT_PDF_V3_STATUS_WINDOW);
    let status = formats::pdf::v3_control::pdf_v3_run_status(
        &dir,
        &run_id,
...
fn synchronize_pdf_v3_run_lifecycle(
    ...
    if let Err(error) = lifecycle.ensure_heartbeat(&run_directory) {
        if !matches!(
            &error,
            formats::pdf::v3_lifecycle::PdfV3RunLifecycleError::Scheduler(
    ...
    let mut refreshed = formats::pdf::v3_control::pdf_v3_run_status(
        job_directory,
        run_id,
        lifecycle.session_id(),
        start_after,
                ...
                diagnostics::append_timeline_event(
                    ...
                    diagnostics::PdfTimelineEvent::new(job_id, "translation", "chunk.completed")
                        ...
                        .details(json!({
                            ...
                            "translationQueueCapacityUnits": output.translation_queue_capacity_units,
                            "translationQueuePeakUnits": output.translation_queue_peak_units,
                            "translationQueuePeakPayloadBytes": output.translation_queue_peak_payload_bytes,
                            "pendingTranslationMapPeakUnits": output.pending_translation_map_peak_units,
                            "pendingTranslationMapPeakChars": output.pending_translation_map_peak_chars,
                            "pendingTranslationMapPeakPayloadBytes": output.pending_translation_map_peak_payload_bytes,
                            "renderPayloadPeakUnits": output.render_payload_peak_units,
                            "renderPayloadPeakChars": output.render_payload_peak_chars,
                            "renderPayloadPeakBytes": output.render_payload_peak_bytes,
                            "combinedPendingPeakUnits": output.combined_pending_peak_units,
                            "combinedPendingPeakChars": output.combined_pending_peak_chars,
                            "combinedPendingPeakPayloadBytes": output.combined_pending_peak_payload_bytes,
